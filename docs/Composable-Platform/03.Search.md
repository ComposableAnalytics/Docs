...
*   *Example:* `Very useful` returns all Resources with the text "very useful" in their name, description, or a tag.
...
*   *Example:* `Very useful type:dataflow` narrows the search to only return DataFlows.
*   *Example:* `name:"My App"` returns Resources of all types with the name "My App", but will not match "My App Rocks" or "MyApp".
*   *Example:* `My App` only contains _Search Text_ and would return resources named "My App Rocks".
*   *Example:* `name:My App` is interpreted as `name:My AND App`, with the _Search Text_ "App" and _Filter_ name:My. The filter would prevent a Resource named "My App" from being returned.
...
*   *Example:* `ClientName moduleInput:true` will return DataFlows having a module with input that contains "ClientName" (such as "ClientName@email.com")
...
*   **Folder** - Limits results to contents of folder `value`. Adding "/*" to a folder name will also search all folders within it.
    ...
    *   *Example:* Suppose a team stores Resources for various projects in separate folders (ProjectA, Project B...) within the folder "/TeamName".  `folder:TeamName/ProjectA` returns Resources in the ProjectA folder.
...
*   **DataFlow only _Filters_**.
    ...
    *   **Refers To** - DataFlows that are directly used by DataFlow with ID `value`. Does not search for nested DataFlow references
...
*   **Age** - DateTime when Resources were created
*   **CloneCount** - Number of times Resources have been cloned.
...
*   *Example:* `type:dataflow sort:name:asc` returns DataFlows sorted alphabetically starting with at the letter A.
*   *Example:* `type:dataflow sort:name:asc sort:age` sorts DataFlows with the same name by age, with the oldest first.
