...
Each time a DataFlow application is executed by a user or through external activation, a `Run` is produced. Composable automatically saves the results for each Module and saves them within a Run. Previous Runs can be accessed through the Run History page, which displays the Run's Times, status, and number of Module Results. The Run History page can be reached from the DataFlow Details page, or in the sidebar when viewing a DataFlow, as explained below.
...
Past Runs can be view on the Run History page, by navigating to it from the Designer using the side navigation bar or from the DataFlow details page.
