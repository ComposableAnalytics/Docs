...
First, add the `QueryView Input` module. The Results of this module are the row(s) of your QueryView. Hovering over the output node shows that it is of type `List<Dictionary<String,Object>>`. This means it is a list where each element is a row of the QueryView. Each row consists of a column name and value in a `<String, Object>` dictionary pair.
