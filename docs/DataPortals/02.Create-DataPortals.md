...
- [**Master (required)**](./03.MasterSheet.md): This is the first sheet that is processed by Composable and acts as a link to all other container sheets that will ultimately be shown to the end user. The [Name](06.Setting-Details/Name.md) field given as a value here will serve as the root database name. Sheet name: `"Master"`.
...
- [**Settings**](./06.SettingSheet.md): These allow for customization of some DataPortal actions. Sheet Name: `<ContainerName>.settings`
...
Once the DataPortal is done building, a blue button will appear that allows you to access the web-based interface.
